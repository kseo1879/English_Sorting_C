...
void num_printer(char **argv) {
    ...
    if(0 == strcmp(argv[2], "-a")) {
        ...
        num_ascending(argv[1]);
    } else {
        //Else it's in decending order.
        num_descending(argv[1]);
    }
    return;
}
    ...
    while(1) {
        ...
        if(0 == strcmp(buf, "\n") || 0 == ftell(fp)) {
            ...
            if(-1 != last_lowest_byte) { //This means it has read the whole file
                if(0 < my_numcmp(file_name, cursur_byte, last_lowest_byte)) {
                    if(-1 == lowest_byte) {
                        lowest_byte = cursur_byte;
                    } else {
                        r_val = my_numcmp(file_name, cursur_byte, lowest_byte);
                        if(0 > r_val) {
                            lowest_byte = cursur_byte;
                        }
                    }   
    ...
    while(1) {
        ...
        if(0 == strcmp(buf, "\n") || 0 == ftell(fp)) {
            ...
            if(-1 != last_biggest_byte) { //This means it has read the whole file
                if(0 > my_numcmp(file_name, cursur_byte, last_biggest_byte)) {
                    // printf("1st%d\n", biggest_byte);
                    if(-1 == biggest_byte) {
                        // printf("2nd%d\n", biggest_byte);
                        ...
                        // printf("3rd%d\n", biggest_byte);
                        r_val = my_numcmp(file_name, cursur_byte, biggest_byte);
